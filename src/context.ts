...
export function getFakeUpstreamInstallation(): FakeUpstreamInstallation {
  ...
  if (!context) {
    throw new Error("No request context found - are you calling this from within a request handler?")
  }
...
export function withContext<T>(context: RequestContext, fn: () => T | Promise<T>): T | Promise<T> {
