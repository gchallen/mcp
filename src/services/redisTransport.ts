...
export async function redisRelayToMcpServer(
  sessionId: string,
  transport: Transport,
  ...
  const subscribe = async (requestId: string) => {
    ...
    redisCleanup = await redisClient.createSubscription(
      toClientChannel,
      async (redisMessageJson) => {
...
export class ServerRedisTransport implements Transport {
  ...
  async send(message: JSONRPCMessage, options?: TransportSendOptions): Promise<void> {
    const relatedRequestId =
      options?.relatedRequestId?.toString() ?? ("id" in message ? message.id?.toString() : notificationStreamId)
...
export async function getShttpTransport(
  sessionId: string,
