...
export function decryptString({ encryptedText, key }: { encryptedText: string; key: string }): string {
...
async function readEncrypted<T>({
  ...
  const data = del ? await redisClient.getDel(prefix + sha256(key)) : await redisClient.get(prefix + sha256(key))
...
export async function saveClientRegistration(clientId: string, registration: OAuthClientInformationFull) {
  await redisClient.set(CLIENT_REGISTRATION_KEY_PREFIX + clientId, JSON.stringify(registration))
}

export async function getClientRegistration(clientId: string): Promise<OAuthClientInformationFull | undefined> {
...
export async function savePendingAuthorization(authorizationCode: string, pendingAuthorization: PendingAuthorization) {
  await saveEncrypted({
...
export async function readPendingAuthorization(authorizationCode: string): Promise<PendingAuthorization | undefined> {
...
export async function saveMcpInstallation(mcpAccessToken: string, installation: McpInstallation) {
  await saveEncrypted({
...
export async function readMcpInstallation(mcpAccessToken: string): Promise<McpInstallation | undefined> {
...
export async function saveRefreshToken(refreshToken: string, mcpAccessToken: string) {
  saveEncrypted({
...
export async function readRefreshToken(refreshToken: string): Promise<string | undefined> {
...
export async function revokeMcpInstallation(mcpAccessToken: string): Promise<void> {
...
export async function saveTokenExchange(authorizationCode: string, tokenExchange: TokenExchange) {
  await saveEncrypted({
 ...
 */
export async function exchangeToken(authorizationCode: string): Promise<TokenExchange | undefined> {
